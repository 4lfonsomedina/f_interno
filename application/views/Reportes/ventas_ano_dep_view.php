...
<div class="col col-md-10">
	<div class="panel panel-default">
		<div class="panel-heading titulo_panel">Ventas anuales</div>

		<div class="panel-body">
			<!--construccion de tabs uno por mes-->
			<div class="row bottom-padding-50 formulario_style">
				<form method="POST" action="<?= site_url('Reportes/ventas/ventas_anuales_dep') ?>" class="form-consulta">
				<div class="col-md-2">
					<b>SUCURSAL</b>
...
<!-- MENSUAL -->
				...
				<tbody>
					...
					<tr>
						<td><?= $pr->producto ?></td><td><?= $pr->desc1 ?></td>
<!-- DIARIO -->
						<?php if($grupo=='diario')for($i=1;$i<13;$i++){
							$dias_del_mes=cal_days_in_month(CAL_GREGORIAN, $i,$ano);
							for($o=1;$o<=$dias_del_mes;$o++){?>
						<td class="importe">
							<?php 
								if(isset($ventas[$i][$o][$pr->producto])) {
									echo number_format($ventas[$i][$o][$pr->producto],2); 
								}
								else echo "0.00";
							?>
						</td>
...
<!-- SEMANAL -->
						<?php if($grupo=='semanal'){
							...
							$dias_del_mes=cal_days_in_month(CAL_GREGORIAN, $i,$ano);
							for($o=1;$o<=$dias_del_mes;$o++){
							if(isset($ventas[$i][$o][$pr->producto]))$total_semanal+=$ventas[$i][$o][$pr->producto];
							$contador_semana++;
...
<!-- MENSUAL -->
						<?php 
							if($grupo=='mensual')for($i=1;$i<13;$i++){
								$dias_del_mes=cal_days_in_month(CAL_GREGORIAN, $i,$ano);
								$total_mensual=0;
								for($o=1;$o<=$dias_del_mes;$o++){if(isset($ventas[$i][$o][$pr->producto]))$total_mensual+=$ventas[$i][$o][$pr->producto];}
						?>
